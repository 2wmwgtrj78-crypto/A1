/* Rendered-geometry and contrast regression test (Playwright, real Chromium).
   ...
   Documented per-check in ARCHIVE_HISTORY.md (originally FINAL_QA_REPORT_2026-09-15-V35.md).
*/
...
async function setVisualMode(page, mode) {
  ...
  /* Setup has NOT been a rail tab since v13: it lives behind More. This line
     queried `.railbtn[data-tab="settings"]`, found nothing, and returned
     silently — so `applied` was always false and all three contrast passes ran
     against whatever mode happened to be active. Route explicitly instead. */
  await page.evaluate(() => { window.SMNAV('settings'); });
...
(async () => {
  ...
  for (const vp of VIEWPORTS) {
    ...
    const g = await page.evaluate(() => {
      /* There is one bar now. smBarTop was retired with the 11-tab layout and is
         not in index.html, so requiring both made this check fail at every
         viewport for reasons unrelated to geometry. */
      const top = document.getElementById('smBarTop'), bot = document.getElementById('smBarBottom');
      if (!bot) return null;
      const tr = top ? top.getBoundingClientRect() : null, br = bot.getBoundingClientRect();
      return {
        top: tr ? { left: Math.round(tr.left), right: Math.round(tr.right), top: Math.round(tr.top), bottom: Math.round(tr.bottom), w: Math.round(tr.width) } : null,
    ...
    check('[' + vp.name + '] the persistent bar exists', !!g);
    if (g) {
      /* The top-bar geometry check was removed with the bar itself; the
         absence check below is what still matters. */
