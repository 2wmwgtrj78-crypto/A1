...
/* Device-local, and correctly NOT backed up. Each needs a reason, so that
   excluding a field is a decision someone made rather than one they forgot.
     ...
     lastSeenVersion — which release's "what changed" note this device has
                   dismissed. Restoring it onto another device would suppress
                   a note that device never actually saw.
     prefs       — exported, but under its own key rather than as st.prefs. */
const DELIBERATELY_LOCAL = ['session', 'visualMode', 'diagnostics', 'lastSeenVersion'];
