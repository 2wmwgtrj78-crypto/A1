...
var baseNav=window.SMNAV;
function goBaseTab(next){ if(typeof baseNav==='function'){ return baseNav(next); } return false; }
...
function tiProgress(ti){
 ...
 var total=lecTotal+bankTotal+speedTotal;
 var done=Math.min(lecTotal,lecDone)+Math.min(bankTotal,bankQ)+Math.min(speedTotal,speedQ);
 return {pct:total?Math.min(100,Math.round(done/total*100)):0,lecDone:lecDone,lecTotal:lecTotal,bankQ:bankQ,bankTotal:bankTotal,speedQ:speedQ,speedTotal:speedTotal};
}
...
function setProgress(v){if(!state.current)return;v=Math.max(0,Math.min(100,Number(v)||0));state.current.progress=v;state.current.partial=v>0&&v<100; if(v>=100)completeBlock();else{save();render();}}
...
function homeHTML(){var r=recommend(),t=topic(r.ti),p=t?tiProgress(t.i):null,done=state.day.blocks||0;return shell('Today','One clear goal. The app chooses the next useful step.',
 '<section class="v17-card hero"><div class="eyebrow">NEXT</div><h2>'+esc(goalText(r))+'</h2><div class="taskmeta"><span>'+esc(labelTask(r.task))+'</span><span>'+esc(effort(r.task))+'</span></div><div class="v17-progress-mini">'+esc(t.n)+' · Topic progress <b>'+p.pct+'%</b></div><div class="bar"><i style="width:'+p.pct+'%"></i></div><div class="v17-sticky-actions actions"><button class="primary" data-start>Start Next</button><button data-break>Break</button><button class="textbtn" data-why>Why this?</button></div></section>'+
 '<section class="v17-card"><div class="eyebrow">TODAY</div><div class="todaybig">'+done+' block'+(done===1?'':'s')+' completed</div><p class="v17-secondary">Block-based progress · '+(state.day.completed||0)+' fully completed.</p></section>');}
function blockHTML(){var c=state.current,r=c.rec,t=topic(r.ti),p=tiProgress(r.ti),pct=Math.round(c.progress||0);return shell('Focus block',t.n,
 ...
 '<div class="progresscontrols"><button data-p="50">Half done</button><button data-p="100">Complete</button></div>'+ (r.task.type==='bank'||r.task.type==='speed'?'<div class="qgrid"><label>Questions attempted<input id="v17Q" type="number" min="0" inputmode="numeric" value="'+(c.questions||'')+'"></label><label>Correct<input id="v17C" type="number" min="0" inputmode="numeric" value="'+(c.correct||'')+'"></label></div>':'')+
 '<p class="autosave">Live saved · Leave anytime; you will resume at '+pct+'%.</p></section>');}
...
function wire(){
 document.querySelectorAll('.railbtn[data-tab]').forEach(function(b){b.onclick=function(){var next=b.dataset.tab;if(next==='today'){view='home';render();}else{goBaseTab(next);}};});
 ...
 document.querySelectorAll('[data-pick]').forEach(function(b){b.onclick=function(){var ti=Number(b.dataset.pick),q=taskFor(ti,(accuracy(ti)!=null&&accuracy(ti)<.7)?'bank':'lecture');if(!q)return;var old=recommend();var reason=prompt('Why are you changing this task?');recordOverride(reason||'Not specified');state.queue.push(old);if(state.queue.length>20)state.queue.shift();start({ti:ti,task:q,reason:'You chose this task.'});};});
