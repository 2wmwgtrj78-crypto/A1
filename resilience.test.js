...
/* This drove the whole suite off `[data-emstate]`, an attribute that is not
   present anywhere in the UI and has not been for many releases. Every click
   was a no-op, so eight checks failed for want of a save rather than for any
   storage reason. The day-tier buttons are the stable repeatable save on
   Today; alternate between them so each call changes state and writes. */
let saveToggle = 0;
async function triggerSave(page) {
  await page.evaluate(() => window.SMNAV && window.SMNAV('today'));
  await page.waitForTimeout(250);
  const n = await page.evaluate(() => document.querySelectorAll('#app [data-layer]').length);
  if (!n) return false;
  const which = (saveToggle++) % n;
  await page.evaluate(i => {
    const els = document.querySelectorAll('#app [data-layer]');
    (els[i] || els[0]).dispatchEvent(new MouseEvent('click', { bubbles: true }));
  }, which);
