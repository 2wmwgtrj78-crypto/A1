/* Dakshinamurthy UI production bundle — 17.0.0. Generated; edit ui-modules instead. */
...
function normalizeState(s){
  ...
  /* Schedule reset: campaign begins 22 Sep 2026. Migrate only the superseded
  ...
  if(s.prefs.startISO==="2026-09-12"||s.prefs.startISO==="2026-09-15"||
     s.prefs.startISO==="2026-09-19"||s.prefs.startISO==="2026-09-20") s.prefs.startISO="2026-09-22";
...
var ICON={
  ...
  /* Phase icons (v16.6). Abstract, not literal anatomy \u2014 same stroke language
     as the set above (20x20, currentColor, 1.6-1.8 weight) rather than a
     different illustration style bolted on. Kept deliberately simple: a
     glyph you learn to recognise at 16px, not a picture you look at. */
  phase1:'<path d="M8 3c-1 2-3 3-3 6a5 5 0 0010 0c0-2-1-3-2-4" fill="none" stroke="currentColor" stroke-width="1.6"/>',
  phase2:'<path d="M10 3v5M10 8l-4 4M10 8l4 4" fill="none" stroke="currentColor" stroke-width="1.6"/><circle cx="6" cy="14" r="1.3" fill="currentColor"/><circle cx="14" cy="14" r="1.3" fill="currentColor"/>',
  phase3:'<path d="M4 8a4 4 0 018 0 4 4 0 008 0" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M4 8v3a4 4 0 008 0" fill="none" stroke="currentColor" stroke-width="1.6"/>',
  phase4:'<path d="M10 3c2 3-2 4 0 7 1.3 1.8 3 1 3 3a3 3 0 01-6 0c0-1.2.8-1.6 1.4-2.4" fill="none" stroke="currentColor" stroke-width="1.6"/>',
  phase5:'<path d="M10 3l6 2v5c0 4-3 6-6 7-3-1-6-3-6-7V5z" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M10 8v5M7.5 10.5h5" stroke="currentColor" stroke-width="1.6"/>',
  phase6:'<circle cx="10" cy="10" r="3.2" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M10 3v2.4M10 14.6V17M3 10h2.4M14.6 10H17" stroke="currentColor" stroke-width="1.6"/>',
  phase7:'<path d="M3 11h3l1.6-3.3L9.5 14 11 8.5l1.3 2.5H17" fill="none" stroke="currentColor" stroke-width="1.6"/>'
};
function icon(k,col){
  return '<svg class="ic" viewBox="0 0 20 20" style="color:'+(col||"currentColor")+'">'+(ICON[k]||ICON.check)+'</svg>';
}
function phaseIcon(n,col){ return icon("phase"+n,col); }
...
function renderToday(){
  ...
  var todayPhase=(entry&&entry.slots&&entry.slots.length)?plan.B.topics[entry.slots[0].ti].phase:null;
  out+='<div class="sm-v25-route"><div class="sm-v25-route-main"><div class="sm-v25-kicker">'+
    (todayPhase?phaseIcon(todayPhase,"#F2C45C"):"")+'Today \u00b7 Your next step</div><div class="sm-v25-copy">'+esc(SM.pretty(date))+(nx?' \u00b7 '+SM.daysBetween(date,nx.iso)+' days to the '+esc(String(nx.style||'exam').toUpperCase()):'')+'</div></div></div>';
...
function quickMcqCard(entry,date){
  ...
  return card('<div class="row-top"><span class="lbl">Log an MCQ</span><span class="muted sm">2 taps</span></div>'+
    ...
    '</div>'+
    /* v16.7: "Right" here always wrote reason:"confident" \u2014 hardcoded, no way
       to say otherwise \u2014 so a guess that happened to land could never be
       flagged by the guess-focus tracking Progress now surfaces globally.
       One more button, not a form: still exactly topic-tap + outcome-tap. */
    '<div class="btnrow" style="margin-top:6px"><button class="btn sm" data-quickresult="guessed">Right, but guessed</button></div>'+
  ...
  rows.forEach(function(row){
    ...
    if(exp){
      ...
      if(["bank","speed","image","repass"].indexOf(b.kind)>=0){
        ...
        /* v16.6: was a plain count-UP stopwatch with no reference to how long
           this block is actually meant to take. Counts down against b.mins
           instead \u2014 the same underlying elapsed time still feeds pace
           tracking below, only the DISPLAY changes \u2014 and flips to a small
           overtime readout past zero rather than showing a nonsense negative
           number. */
        var targetSecs=(b.mins||0)*60, remain=targetSecs-el, over=remain<0;
        var showSecs=Math.abs(remain);
        inner+='<div class="timer"><span class="tclock'+(over?" rust":"")+'">'+
          (over?"+":"")+String(Math.floor(showSecs/60)).padStart(2,"0")+':'+String(showSecs%60).padStart(2,"0")+
          '<i class="muted sm" style="font-style:normal;margin-left:6px">'+(over?"over":"left")+'</i></span>'+
...
/* openedAt: v16.6. Marks when THIS topic's log panel opened, not when the
   question was first read in the source app \u2014 there is no way to observe
   that from inside this app. It is dwell time on the log entry (choosing the
   subtopic/number, picking an outcome, giving a reason), not pure
   question-solving time, and is presented as exactly that everywhere it is
   shown \u2014 never claimed to be more precise than it actually is. */
function qlogReset(){ qlog={ti:null,app:"DocTutorials",pool:"bank",subtopic:"",number:"",stage:null,conf:null,errType:null,chose:"",retest:"",openedAt:Date.now()}; }
var qAnalyse=null;
var searchQ="";
...
function qUndoApply(){
  ...
  qlog.stage=null; qlog.conf=null; qlog.errType=null; qlog.chose=""; qlog.retest=""; qlog.openedAt=Date.now();
  qUndo=null;
  return true;
}
function qUndoBar(){
...
function renderPlan(){
  ...
  out+=card('<div class="eyebrow">What comes when</div>'+
    [1,2,3,4,5,6,7].map(function(k){
      ...
      return '<div class="trow"><div class="row-top"><span class="lbl">'+phaseIcon(k,"var(--drape)")+k+'. '+esc(PN[k]||("Phase "+k))+'</span>'+
